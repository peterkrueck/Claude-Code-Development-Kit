---
name: second-opinion
description: Get a second opinion from Google's Gemini Pro via the locally installed Gemini CLI (defaults to gemini-3.1-pro-preview; override with the CLAUDE_SECOND_OPINION_MODEL env var). Use this skill when in Plan Mode for large or critical tasks, when stuck on a debugging dead end, when facing architecture trade-offs, for subtle edge cases in code review, or any situation where an independent perspective would add value. Also use when the user explicitly asks for a "second opinion", "another perspective", "cross-check this", "ask Gemini", or "Gemini's take". Reports unavailability rather than falling back to a weaker model.
user_invocable: false
---

<!-- This is the DEFAULT-engine form of the Gemini second-opinion skill.
     setup.sh installs it as skills/second-opinion/SKILL.md when the user picks
     Gemini as their default second-opinion engine. The narrow sibling
     second-opinion-gemini/SKILL.md is the explicit-only variant used when Codex
     (or "both") is the default. Keep the two bodies in sync; only the
     frontmatter description and the "is/isn't the default" framing differ. -->
...
- When the user explicitly asks for a second opinion, "another perspective", or "cross-check this"

Don't use this for routine tasks — every call takes time and has capacity limits. Reserve it for decisions where being wrong has real consequences.
...
**If "ask both":** If a second independent engine is installed, present that engine's view and Gemini's view side by side, then your synthesis across all three perspectives (yours + both outside models) — flag where the two outside models agree (high confidence) vs. where they diverge (the real decision point).

**If Gemini is unavailable — capacity exhausted / 429:**

> Gemini Pro is currently at capacity. No fallback to a weaker model — only Pro-tier is capable enough for meaningful second opinions, and the value is an independent architecture. Proceeding with my own analysis only.
